...
use crate::{
    AllocInit,
    common::{
        ALLOCATOR_ALREADY_INITIALIZED, Alloc, BAllocator, BAllocatorError, HEAP_END_OVERFLOWED,
        HEAP_NOT_POWER_TWO, HEAP_SIZE_ZERO, HEAP_START_NULL, align_up,
    },
};
        ...
        loop {
            ...
            if head.is_null() {
                    ...
                    .is_ok()
                {
                    ...
                    break Some(unsafe { NonNull::new_unchecked(head) });
                }
            }
        }
    }
...
impl LocklessBuddy {
    ...
    fn split_area_to(&self, target_order: usize) -> Result<(), BAllocatorError> {
        ...
        for current_order in (target_order..=source_order).rev() {
            if self.list_areas[current_order]
                .nr_free
                ...
                > 0
            {
                ...
                let area = loop {
                    match self.list_areas[current_order].pop() {
